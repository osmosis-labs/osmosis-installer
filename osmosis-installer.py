...
def initSetup ():
    ...
    #subprocess.run(["echo 'export GOPATH=\""+ HOME.stdout.strip()+"/go\"' >> "+ HOME.stdout.strip()+"/.profile"], shell=True) 
    #subprocess.run(["echo 'export GOBIN=\"$GOPATH/bin\"' >> "+ HOME.stdout.strip()+"/.profile"], shell=True)
    #subprocess.run(["echo 'export PATH=\"$GOBIN:$PATH\"' >> "+ HOME.stdout.strip()+"/.profile"], shell=True) 
