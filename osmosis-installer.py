...
3) Don't install cosmovisor and don't set up a background service
    ...
    if useCosmovisor == "1":
        ...
        subprocess.run(["git checkout v7.1.0"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
...
def initSetup ():
    ...
    if os_name == "Linux":
        ...
        os.chdir(os.path.expanduser(HOME+'/osmosis'))
        if networkAns == "1":
            print(bcolors.OKGREEN + "(5/5) Installing Osmosis v7.1.0 Binary..." + bcolors.ENDC)
            ...
            subprocess.run(["git checkout v7.1.0"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        if networkAns == "2":
            print(bcolors.OKGREEN + "(5/5) Installing Osmosis v7.1.0 Binary..." + bcolors.ENDC)
            subprocess.run(["git stash"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
            subprocess.run(["git pull"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        ...
        print(bcolors.OKGREEN + "(4/4) Installing Osmosis V7.1.0 Binary..." + bcolors.ENDC)
        os.chdir(os.path.expanduser(HOME))
        subprocess.run(["git clone https://github.com/osmosis-labs/osmosis"], shell=True)
        os.chdir(os.path.expanduser(HOME+'/osmosis'))
        subprocess.run(["git stash"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        subprocess.run(["git checkout v7.1.0"], shell=True)
        my_env["PATH"] = "/"+HOME+"/go/bin:/"+HOME+"/go/bin:/"+HOME+"/.go/bin:" + my_env["PATH"]
