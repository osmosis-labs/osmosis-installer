...
3) Don't install cosmovisor and don't set up a background service
    ...
    if useCosmovisor == "1":
        ...
        subprocess.run(["git checkout v10.1.1"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
...
def initSetup ():
    ...
    if os_name == "Linux":
        ...
        subprocess.run(["git pull"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        if networkAns == "1":
            print(bcolors.OKGREEN + "(5/5) Installing Osmosis v10.1.1 Binary..." + bcolors.ENDC)
            subprocess.run(["git checkout v10.1.1"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        if networkAns == "2":
            print(bcolors.OKGREEN + "(5/5) Installing Osmosis v11.0.0 Binary..." + bcolors.ENDC)
            subprocess.run(["git checkout v11.0.0"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        if networkAns == "3":
        ...
        subprocess.run(["git pull"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        if networkAns == "1":
            print(bcolors.OKGREEN + "(4/4) Installing Osmosis 10.0.1 Binary..." + bcolors.ENDC)
            subprocess.run(["git checkout v10.1.1"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        elif networkAns == "2":
            print(bcolors.OKGREEN + "(4/4) Installing Osmosis v11.0.0 Binary..." + bcolors.ENDC)
            subprocess.run(["git checkout v11.0.0"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        elif networkAns == "3":
...
Welcome to the Osmosis node installer v10.1.1!
